...
public class SolarSystemController : MonoBehaviour
{
	...
	void Start()
	{
		...
		m_OrbitPlots[i++] = SpawnOrbit(  0, 0.2056, 5.79091e7, 7.006,  29.12,  48.34, 252.25, m_Sun); // Mercury
		m_OrbitPlots[i++] = SpawnOrbit(290, 0.0068, 1.08209e8, 3.398,  54.88,  76.67, 181.98, m_Sun); // Venus
		m_OrbitPlots[i++] = SpawnOrbit(190, 0.0167, 1.49598e8, 0.000, 114.21,   0.00, 100.47, m_Sun); // Earth
		m_OrbitPlots[i++] = SpawnOrbit(120, 0.0055, 3.84748e5, 5.100,      0,   0.00,      0, m_Earth); // Moon
		m_OrbitPlots[i++] = SpawnOrbit( 20, 0.0934, 2.27940e8, 1.852, 286.50,  49.71, 355.43, m_Sun); // Mars
		m_OrbitPlots[i++] = SpawnOrbit( 40, 0.0489, 7.78478e8, 1.299, 273.87, 100.29,  34.33, m_Sun); // Jupiter
		m_OrbitPlots[i++] = SpawnOrbit( 60, 0.0565, 1.43354e9, 2.494, 339.39, 113.64,  50.08, m_Sun); // Saturn
		m_OrbitPlots[i++] = SpawnOrbit(160, 0.0472, 2.87097e9, 0.077,  97.00,  73.96, 314.20, m_Sun); // Uranus
		m_OrbitPlots[i++] = SpawnOrbit(210, 0.0087, 4.49841e9, 1.770, 273.19, 131.79, 304.22, m_Sun); // Neptune
	...
	void UpdatePlanetPositions(double atTime) {
		...
		for (int i = 0; i < m_PlanetCount; i++) {
			...
			if (i == 2) { // Special case for Moon-Earth system
				GameObject moon = m_OrbitPlots[3];
